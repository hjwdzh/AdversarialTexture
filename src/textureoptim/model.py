...
EPS = 1e-12
Model = collections.namedtuple("Model", "predict_real,predict_fake,predict_mask,discrim_loss,discrim_loss_final,gen_loss_GAN,gen_loss_L1,outputs,texture,train,l1_weight,global_step")
...
def create_model(data,lr_D,lr_G,beta1,initial_file):
    ...
    def create_discriminator(input, mask):
        ...
        for i in range(n_layers):
            with tf.variable_scope("layer_%d" % (len(layers) + 1)):
                ...
                convolved_mask = discrim_conv_mask(layers_mask[-1], stride=stride)
                rectified = lrelu(convolved, 0.2)
                layers.append(rectified)
                layers_mask.append(convolved_mask)
    ...
    l1_weight = float(10.0) * (float(0.8) ** tf.dtypes.cast((global_step//960),tf.float32))
    texture = create_texture(initial_file)
    outputs = tf.contrib.resampler.resampler(texture, data.uv_src, name='resampler')
    ...
    with tf.name_scope("real_discriminator"):
        with tf.variable_scope("discriminator"):
            ...
            predict_real, mask_real = create_discriminator(input[:,sy:,sx:,:], data.mask[:,sy:,sx:,:])
    ...
    with tf.name_scope("fake_discriminator"):
        with tf.variable_scope("discriminator", reuse=True):
            # 2x [batch, height, width, channels] => [batch, 30, 30, 1]
            predict_fake, mask_fake = create_discriminator(input[:,sy:,sx:,:], data.mask[:,sy:,sx:,:])

    style_weight = 1e-6
    with tf.name_scope("generator_loss"):
        gen_loss_GAN = tf.reduce_mean(-tf.log(predict_fake + EPS))
        gen_loss_L1 = tf.reduce_sum(tf.reduce_sum(tf.abs(data.color_tar[:,sy:,sx:,:] - outputs[:,sy:,sx:,:]),axis=3) * data.mask[:,sy:,sx:,0]) / (tf.reduce_sum(data.mask[:,sy:,sx:]) * float(3.0) + EPS)
        gen_loss = gen_loss_L1 * l1_weight + gen_loss_GAN

    with tf.name_scope("discriminator_loss"):
        discrim_loss = tf.reduce_sum((-(tf.log(predict_real + EPS) + tf.log(1 - predict_fake + EPS))) * mask_real) / (tf.reduce_sum(mask_real) + EPS)

    with tf.name_scope("discriminator_train"):
        discrim_tvars = [var for var in tf.trainable_variables() if var.name.startswith("discriminator")]
        discrim_optim = tf.train.AdamOptimizer(lr_D, beta1)
        discrim_loss_final = tf.cond(discrim_loss > gen_loss_GAN, lambda: discrim_loss, lambda: discrim_loss * 0)
        discrim_grads_and_vars = discrim_optim.compute_gradients(discrim_loss_final, var_list=discrim_tvars)
        discrim_train = discrim_optim.apply_gradients(discrim_grads_and_vars)

    with tf.name_scope("generator_train"):
        with tf.control_dependencies([discrim_train]):
            gen_tvars = [var for var in tf.trainable_variables() if var.name.startswith("texture")]
